...
void Dashboard::createMeter(uint8_t x,uint8_t y,uint8_t r){
  ...
  _y=y;
  _r=r;
  M5.Lcd.drawCircleHelper(x, y, r, 0x3, TFT_GREEN);
}

void Dashboard::updateMeter(float val){
  //150 + 60 * sin(ins.roll() * 6.28 / 360), 148 - 60 * cos(ins.roll() * 6.28 / 360
  M5.Lcd.drawLine(_x, _y, _x + (_r-10) * sin(_val * 6.28 / 360), _y - (_r-10) * cos(_val * 6.28 / 360), TFT_BLACK);
  M5.Lcd.drawLine(_x, _y, _x + (_r-10) * sin(val * 6.28 / 360), _y - (_r-10) * cos(val * 6.28 / 360), TFT_GREEN);
  _val=val;
}
